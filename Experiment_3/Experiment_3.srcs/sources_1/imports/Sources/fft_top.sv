...
logic trigger_d, trigger_fall;

always_ff @(posedge clk_i or negedge arstn) begin
    if (!arstn) begin
        trigger_d    <= 1'b1;  // Assuming trigger starts high
        trigger_fall <= 1'b0;
    end else begin
        trigger_fall <= trigger_d && !trigger;  // One-cycle pulse on falling edge
        trigger_d    <= trigger;
    end
end
...
assign config_data.pad = 5'd0; //critcal 
